...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if(start==null || end==null || !containsVertex(start) || !containsVertex(end) || start.equals(end)){
        ...
        if(containsVertex(start) && containsVertex(end)){
            ...
            while(!priorityQueue.isEmpty()){
                Path shortestPath = priorityQueue.poll();
                if(shortestPath.end.data.equals(end)){
                    //if shortestPath's end data equals to end data, which means the whole path
                    //is the shortestPath
                    return shortestPath;
                }
                for(Edge e : shortestPath.end.edgesLeaving){
                    //iterate all paths, add the shortest path
                    path = new Path(shortestPath, e);
                    priorityQueue.add(path);
                }
            }
        }
        // if no shortest path found then throw exception
        throw new NoSuchElementException();
    }
